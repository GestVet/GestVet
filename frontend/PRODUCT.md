...
Binding decision by the user (2026-09-12): the UI is built with **shadcn/ui on Tailwind CSS**, and every table uses **TanStack Table**. shadcn/ui (Radix base) and Tailwind CSS 4 are installed and every screen is built with them; there is no hand-written CSS left. Every listing uses TanStack Table 9 through the shared `DataTable` component. Visual identity decided by the user (2026-09-12): the shadcn tokens carry GestVet's existing palette (institutional blue `#004b8d` as primary, green for affirmative actions, the soft gray-blue ground), keeping the identity of the original interface. Where an original color fails WCAG 2.2 AA it is darkened just enough (green `#1e7b34` for text on or over it, muted text `#5c6370`, field borders `#7f8a98`). There is no dark theme. Adopting shadcn/ui must respect the project's enforced lint rules (file naming, architecture boundaries, no barrel files, comment policy) by adapting configuration, not by adding inline exceptions.
