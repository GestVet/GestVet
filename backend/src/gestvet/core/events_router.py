...
from gestvet.core.auth import StreamPrincipalDep
from gestvet.core.logs import get_logger
from gestvet.core.realtime_broker import BrokerDep

# Cuánto espera el navegador para reconectar si se corta la conexión.
RETRY_MS = 3000

router = APIRouter()
logger = get_logger("gestvet.realtime")
...
async def stream_events(
    ...
    async with broker.subscribe() as queue:
        logger.info("realtime.connected", user_id=principal.user_id, role=principal.role.value)
        try:
