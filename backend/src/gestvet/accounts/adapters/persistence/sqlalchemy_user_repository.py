"""Implementación del puerto `UserRepository` sobre SQLAlchemy."""
...
class SqlAlchemyUserRepository:
    ...
    async def search_by_role(self, role: Role, query: ClientQuery) -> Page[User]:
        base = select(UserRow).where(UserRow.role == role.value)
    ...
    def _apply_filters(self, statement: Select[tuple[UserRow]], query: ClientQuery):
        if query.search:
