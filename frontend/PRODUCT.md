...
Binding decision by the user (2026-09-12): the UI is built with **shadcn/ui on Tailwind CSS**, and every table uses **TanStack Table**. shadcn/ui (Radix base) and Tailwind CSS 4 are installed and configured; TanStack Table is installed but no screen uses it yet. The existing screens still use the hand-written CSS in `src/style.css`, kept in a `legacy` cascade layer until they migrate. Visual identity decided by the user (2026-09-12): the shadcn tokens carry GestVet's existing palette (institutional blue `#004b8d` as primary, green for affirmative actions, the soft gray-blue ground), so migrated and unmigrated screens stay coherent. Where an original color fails WCAG 2.2 AA it is darkened just enough (green `#1e7b34` for text on or over it, muted text `#5c6370`, field borders `#7f8a98`). There is no dark theme. Screens migrate to shadcn one area at a time; the sign-in, registration and password recovery screens are the first. Adopting shadcn/ui must respect the project's enforced lint rules (file naming, architecture boundaries, no barrel files, comment policy) by adapting configuration, not by adding inline exceptions.
